...
import { getDefaultLocationSlug, getSlugForId, isBakeryRestaurant, menuPath } from "@/lib/restaurants";
...
export default function CheckoutPage() {
  ...
  const isBakery = isBakeryRestaurant(RESTAURANT_ID);
  const menuBrowseHref = menuPath(
    getSlugForId(activeRestaurantId) || getDefaultLocationSlug() || "praca-do-chile"
  );
  const localizedMenuHref = lp(menuBrowseHref);
  const { openingSlots } = useOrderingHours();
  const orderingOpenNow =
  ...
  const orderingClosedNow = !orderingOpenNow;
  ...
  const [whenMode, setWhenMode] = useState(
    isBakery || !orderingOpenNow ? "schedule" : "asap"
  );
  const [scheduleDate, setScheduleDate] = useState("");
  const [scheduleTime, setScheduleTime] = useState("");

  const scheduleDateOptions = useMemo(
    () => buildOrderScheduleDateOptions(openingSlots || [], 7, DEFAULT_RESTAURANT_TIMEZONE),
    [openingSlots]
  );
  ...
  useEffect(() => {
    if (isBakery || !orderingOpenNow) setWhenMode("schedule");
  }, [orderingOpenNow, isBakery]);

  useEffect(() => {
    if (whenMode !== "schedule") return;
    if (!scheduleDate && scheduleDateOptions.length > 0) {
      setScheduleDate(scheduleDateOptions[0].value);
    }
  }, [whenMode, scheduleDate, scheduleDateOptions]);

  useEffect(() => {
    if (whenMode !== "schedule" || !scheduleDate) return;
    if (scheduleTime && scheduleTimeOptions.includes(scheduleTime)) return;
    setScheduleTime(scheduleTimeOptions[0] || "");
  }, [whenMode, scheduleDate, scheduleTime, scheduleTimeOptions]);
      ...
      <div className="min-h-screen bg-zinc-50 text-zinc-900 dark:bg-zinc-950 dark:text-zinc-100">
        ...
        <main className="mx-auto max-w-2xl px-4 py-12">
          <p className="text-center text-zinc-600 dark:text-zinc-300">
            Your cart is empty. <Link href={localizedMenuHref} className="font-medium text-amber-700 underline hover:text-amber-600 dark:text-amber-400 dark:hover:text-amber-300">Browse menu</Link>
          </p>
        </main>
      </div>
    );
          ...
          <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-8 text-center text-emerald-950 dark:border-emerald-700 dark:bg-emerald-950/50 dark:text-emerald-50">
            ...
            <p className="mt-2 text-emerald-800 dark:text-emerald-200">
              {paymentMethod === "cash_on_delivery"
                ? "Pay when you pick up your takeaway order."
                : "Your payment has been processed."}
            </p>
            {!isAuthenticated && (
              <p className="mt-3 text-sm text-emerald-800/90 dark:text-emerald-200/90">
                To track this order later, <Link href={lp("/login")} className="font-medium underline hover:no-underline">log in</Link> with the same email.
              </p>
            )}
            <Link
              href={localizedMenuHref}
  ...
  const orderItems = items.map(({ item, quantity }) => {
    ...
    if (variantId != null && variantId !== "" && !Number.isNaN(Number(variantId))) {
  ...
  function buildOrderPayload(extra = {}) {
    ...
    if (whenMode === "schedule" && scheduleDate && scheduleTime) {
      payload.scheduled_for = formatScheduledForPayload(scheduleDate, scheduleTime);
    }
    return payload;
  }
  ...
  async function handlePlaceOrder(e) {
    ...
    if (whenMode === "asap" && orderingClosedNow) {
      setError("ASAP takeaway is only available during opening hours. Please schedule a pickup time.");
      return;
    }
    setSubmitting(true);
    try {
      ...
      <main className="mx-auto max-w-2xl px-4 py-8 text-zinc-900 dark:text-zinc-100 md:py-12">
        ...
        {orderingClosedNow && (
          <div className="mb-6 rounded-xl border border-amber-300 bg-amber-50 p-4 text-amber-950 dark:border-amber-700 dark:bg-amber-950/40 dark:text-amber-50">
            <p className="font-medium">Kitchen closed for ASAP takeaway</p>
            <p className="mt-1 text-sm text-amber-900/90 dark:text-amber-100/90">
              Schedule a pickup time below — takeaway only, no delivery.
            </p>
          </div>
        )}
        ...
        <form onSubmit={handlePlaceOrder} className="space-y-6 text-zinc-900 dark:text-zinc-100">
          ...
          <div>
            <p className="mb-2 text-sm font-medium text-zinc-700 dark:text-zinc-300">
              {isBakery
                ? t(messages, "checkout.bakeryPickup", "Cake & bakery pickup")
                : t(messages, "checkout.takeawayOnly", "Takeaway pickup only")}
            </p>
            <p className="text-xs text-zinc-500 dark:text-zinc-400">
              {isBakery
                ? "Collect your order at the bakery. Scheduled pickup is recommended for cakes."
                : "Collect your order at the restaurant. Delivery is not available online."}
            </p>
          </div>

          <div>
            <label className="mb-2 block text-sm font-medium text-zinc-700 dark:text-zinc-300">When</label>
            <div className="flex gap-2">
              <button
                type="button"
                disabled={!orderingOpenNow}
                onClick={() => setWhenMode("asap")}
                className={`flex-1 rounded-xl border px-4 py-3 text-sm font-medium transition-colors disabled:cursor-not-allowed disabled:opacity-40 ${
                  whenMode === "asap"
            ...
            {whenMode === "schedule" && (
              <div className="mt-3 grid gap-3 sm:grid-cols-2">
                <div>
                  <label className="mb-1 block text-xs font-medium text-zinc-600 dark:text-zinc-400">Date *</label>
                  <select
                    value={scheduleDate}
                    onChange={(e) => setScheduleDate(e.target.value)}
                    required
                    className="h-12 w-full rounded-xl border border-zinc-300 px-3 text-sm dark:border-zinc-700 dark:bg-zinc-900"
                  >
                ...
                <div>
                  <label className="mb-1 block text-xs font-medium text-zinc-600 dark:text-zinc-400">Time *</label>
                  <select
                    value={scheduleTime}
          ...
          {!isAuthenticated && (
            <p className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-2 text-sm text-amber-950 dark:border-amber-700 dark:bg-amber-950/60 dark:text-amber-50">
              ...
              <span className="text-amber-900/90 dark:text-amber-100/95">To track your order later, <Link href={lp("/login")} className="font-semibold underline hover:no-underline">log in</Link> before or after placing it.</span>
            </p>
          )}
          <div>
          ...
          <button
            type="submit"
            disabled={submitting || !gdprConsent || (whenMode === "asap" && orderingClosedNow)}
          ...
          >
            {submitting
              ? "Placing order…"
              : whenMode === "schedule"
                ? `${isBakery ? t(messages, "checkout.scheduleCake", "Schedule pickup") : t(messages, "checkout.scheduleOrder", "Schedule takeaway")} · ${formatPrice(totalAmount)}`
